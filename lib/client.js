 ...
 *     <li><b>requireTLS</b> - require TLS server support</li>
...
SMTPClient.prototype._init = function(){
    ...
    this._requireTLS = !!this.options.requireTLS;
...
SMTPClient.prototype._authenticateUser = function(){
    // Error out if we require TLS but don't have it
    if (this._requireTLS && !this._secureMode){
      this._onError(new Error("Server doesn't support TLS but we require it"), false);
      return;
    }
